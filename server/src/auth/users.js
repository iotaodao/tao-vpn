...
/**
 * Resolve identity → existing user OR invite-backed new user.
 * identity: { telegram_id?, telegram_handle?, phone?, email? }
...
export function loginOrRegister({ identity, method, deviceLabel, userAgent, ip }) {
  ...
  // 2) Otherwise need an invite that matches identity
  if (!user) {
    let invite;
    ...
    const userRow = {
      id: userId,
      name: invite.name,
      telegram_id: identity.telegram_id || invite.telegram_id || null,
      telegram_handle: identity.telegram_handle || null,
      phone: identity.phone || invite.phone || null,
      email: identity.email || invite.email || null,
  ...
  insertSession.run({
    ...
    created_at: ts,
    last_seen_at: ts,
    expires_at: ts + SESSION_TTL_DAYS * 86400_000,
  });
