...
export function pollTgAuth(state) {
  ...
  return { status: "confirmed", identity: { telegram_id: row.telegram_id, telegram_handle: row.telegram_user, name: row.telegram_name } };
}
...
export async function startBot() {
  ...
  // Очищаем зависшие вебхуки перед стартом
  ...
  bot.command("start", async (ctx) => {
    ...
    const r = confirmState.run(String(u.id), [u.first_name, u.last_name].filter(Boolean).join(" "), u.username || null, now(), state, now());
    if (r.changes === 0) {
      await ctx.reply("Ссылка устарела.");
    } else {
      await ctx.reply("✅ Подтверждено. Вернитесь в приложение.");
    }
  });

  bot.catch((err) => console.error("[tgbot] Runtime error:", err));
...
export function botUsername() { return process.env.TELEGRAM_BOT_USERNAME || "TaoVPNBot"; }
